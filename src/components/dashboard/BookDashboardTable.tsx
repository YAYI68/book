...
const BookDashboardTable = (props: Props) => {
  ...
  return (
    <div className='w-full h-full flex flex-col gap-2'>
         ...
         <div className='w-full overflow-x-scroll mt-[2rem]'>
           <Table>
             <BookTableHeader selectedValue={data} setSelectAllRow={setSelect} />
              {data.map((num,i)=>(
            <BookTableBody key={i} rowId={num} selectedNum={select} setSelect={setSelect} />
          ))}
           </Table>
         </div>
...
export default BookDashboardTable