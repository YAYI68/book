...
const Profile = async (props: Props) => {
  ...
  return (
    <div className="w-full flex flex-col gap-4 items-center lg:items-start lg:flex-row">
      <div className="w-[14rem] relative h-[14rem] lg:w-[20%] lg:justify-between lg:h-[15rem] rounded-[50%] bg-green-500 overflow-hidden">
        <Image
      ...
      <div className="flex w-full  flex-col lg:w-[70%]">
        ...
        <div className="w-full py-2 flex flex-col lg:flex-row lg:flex-wrap lg:justify-between">
          ...
          <ProfileDetail name="Book Read" content={profile.readCount} />
          <ProfileDetail name="Book Download" content={profile.downloadCount} />
          <ProfileDetail
            name="Account Status"
            content={profile.is_active ? "Active" : "Not Active"}
          />
        </div>
      </div>
    </div>
