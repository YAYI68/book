...
const Theme = (props:Props) => {
  ...
  return (
    ...
    <button onClick={()=>onChangeTheme('light')} className={`  ${activeTheme==="light"?'text-red-600':''} w-full p-2 flex items-center gap-2 hover:bg-slate-500 `}>
     <span><LightIcon classname='h-[1rem] w-[1rem]' /></span><span>Light</span>
    </button>
    <button  onClick={()=>onChangeTheme('system')} className={` ${activeTheme==="system"?'text-red-600':''} w-full p-2 flex items-center  gap-2 hover:bg-slate-500 `}>
