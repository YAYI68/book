...
export async function PATCH(req: Request,{ params }) {
   ...
   try{
     ...
     if((image && image.include(CLOUDINARY_URL)) || (note && note.include(CLOUDINARY_URL))){
         const book = await Book.findByIdAndUpdate({_id:params.id},{
      ...
      console.log({deletedImage,deletedNote})
      if(deletedImage==="ok" || deletedNote==="ok" ){
